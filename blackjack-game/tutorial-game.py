...
def draw_scores(player, dealer):
    screen.blit(font.render(f'Score[{player}]', True, 'white'), (300, 400))
    if reveal_dealer:
        screen.blit(font.render(f'Score[{dealer}]', True, 'white'), (300, 100))
    else:
        screen.blit(font.render(f'Score[???]', True, 'white'), (300, 100))
...
def calculate_score(hand):
    # calculate hand score, fresh every time and check how many aces we have
    hand_score = 0
...
run = True
while run:
    # run game at our framerate and fill screen with bg colour
    timer.tick(fps)
    screen.fill('black')
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            if not active:
                ...
                # if player can hit, allow them to draw card
